...
export default function AdminAddressModal() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch("http://localhost:8000/address/create", {
                method: "POST",
                headers: {
            ...
            if (data.success) {
                ...
                await log("error", "add new item address error", data);
            }
        } catch (error) {
            await log("error", "add new item address error", error);
        }
    };
