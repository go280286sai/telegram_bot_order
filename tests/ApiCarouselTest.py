...
async def test_create_api_carousel():
    response = client.post(f"http://127.0.0.1:8000/front/carousel/create",
...
async def test_update_api_carousel():
    response = client.post(f"http://127.0.0.1:8000/front/carousel/update/1",
                           headers={"Content-Type": "application/json"},
                           json={
                               "title": "Title2",
...
async def test_get_api_carousel():
    response = client.get(f"http://127.0.0.1:8000/front/carousel/gets")
    assert response.status_code == 200
    data = response.json()
    assert data['success'] is True
...
async def test_delete_api_carousel():
    response = client.post(f"http://127.0.0.1:8000/front/carousel/delete/1")
    assert response.status_code == 200
    data = response.json()
    assert data['success'] is True
    assert data['data'] is None
