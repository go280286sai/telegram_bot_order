...
async def test_create_delivery():
    async with async_session_maker() as session:
        ...
        delivery = await delivery_manager.create_delivery(name=NAME, city=CITY, address=ADDRESS)
...
async def test_update_deliveries():
    async with async_session_maker() as session:
        delivery_manager = DeliveryManager(session)
        delivery = await delivery_manager.update_delivery(1, name=NAME+"1", city=CITY+"1", address=ADDRESS+"1")
        assert delivery is True
